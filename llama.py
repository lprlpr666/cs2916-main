...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        # todo
            # 计算均方（Mean Squared Value）
        ms = torch.mean(x.pow(2), dim=-1, keepdim=True)
        # 在均方值上加上epsilon以保证数值稳定性，然后计算RMS
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        return context
        # todo
        # q_k_product = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(query.size(-1))

        # # 2. Apply softmax to get the attention weights
        # # Shape of attn_weights: (bs, n_local_heads, seqlen, seqlen)
        # attn_weights = F.softmax(q_k_product, dim=-1)
        
        # # 3. Apply dropout to the attention weights
        # attn_weights = self.attn_dropout(attn_weights)

        # # 4. Multiply the attention weights with the value tensor
        # # Shape of attn_output: (bs, n_local_heads, seqlen, head_dim)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        # todo
        # 1) Apply RMS layer normalization to the input
        x_norm = self.attention_norm(x)
        
        # 2) Compute self-attention on the layer-normalized input
        attn_output = self.attention(x_norm)
        
        # 3) Add the input to the output of the self-attention (residual connection)
        x = x + attn_output
        
        # 4) Apply layer normalization to the output of the self-attention
        x_norm = self.ffn_norm(x)
        
        # 5) Pass the layer-normalized output of self-attention through a feed-forward network
...
class Llama(LlamaPreTrainedModel):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            # todo
            # idx_next = torch.multinomial(F.softmax(logits / temperature, dim=-1), num_samples=1)
            ...
            if temperature == 0.0:
                ...
                idx_next = logits.argmax(dim=-1, keepdim=True)  # Greedy selection of the next index
            else:
                '''
                ...
                '''
                # idx_next = None
                # Scale the logits by the inverse of the temperature
