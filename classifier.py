...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		'''
		# todo
		# Step 1: 通过Llama模型获取输入序列的上下文化表示
		# logits的形状是(batch_size, sequence_length, vocab_size)，我们只需要序列最后一个标记的表示
		_, hidden_states = self.llama(input_ids)
		final_hidden_state = hidden_states[:, -1, :]

		# Step 2: 对最后一个隐藏状态应用dropout
		# 这有助于在训练过程中减少对特定训练实例的过拟合
		final_hidden_state = self.dropout(final_hidden_state)

		# Step 3: 将dropout后的隐藏状态通过分类器头得到类别的逻辑值
		logits = self.classifier_head(final_hidden_state)

		# Step 4: 计算并返回所有类别上的对数概率
		# 这对于后续的交叉熵损失计算或多类别分类任务非常有用
		log_probabilities = F.log_softmax(logits, dim=-1)
